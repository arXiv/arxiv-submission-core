...
def _apply_events(events: List[Event], rules: List[EventRule],
         ...
         -> List[Tuple[Submission, List[Event]]]:
    """
    ...
    full_stack: List[Tuple[Submission, List[Event]]] = []

    def _apply_event(ev: Event, s: Submission) -> Submission:
        try:
            ev.validate(s)
        except InvalidEvent as e:
            invalid_events.append(e)
            return
        next_state = ev.project(s)

        if next_state.submission_id is None and s.submission_id is not None:
            classic.store_events(applied_events, submission=s)
            full_stack.append((s, list(applied_events)))
            applied_events.clear()
        applied_events.append(ev)
        return next_state

    for event in events:
        submission = _apply_event(event, submission)

        if event.committed:   # Don't create duplicate rule-derived events.
            continue

        # Any rule-derived events should be applied before moving on.
        extra_events = _apply_rules(submission, event, rules)
        for extra_event in extra_events:
            submission = _apply_event(extra_event, submission)
            extra_events.append(extra_event)
